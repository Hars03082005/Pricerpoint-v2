...
CONDITION_MULTIPLIERS = METADATA.get("condition_handling", {}).get("condition_multipliers", {"excellent": 1.035, "good": 1.0, "average": 0.94, "poor": 0.86})
...
def build_features(vehicle: VehicleInput) -> pd.DataFrame:
    ...
    km_per_year = km / max(vehicle_age, 1)
...
def condition_multiplier(condition: str) -> float:
    return float(CONDITION_MULTIPLIERS.get(clean_text(condition, "good"), CONDITION_MULTIPLIERS.get("good", 1.0)))


def predict_base_market_value(vehicle: VehicleInput) -> int:
    """Ensemble base market value prediction from uploaded-dataset features."""
    ...
    return int(round(market_value / 500) * 500)


def predict_market_value(vehicle: VehicleInput) -> dict:
    """Return base ML value and final condition-calibrated market value.

    The dataset does not contain verified condition labels, so condition is applied as a
    monotonic calibration after ensemble prediction. This fixes the issue where Poor
    condition could occasionally score higher than Average for the same car.
    """
    base_value = predict_base_market_value(vehicle)
...
def _wheelr_enrichment(
    *,
    ...
    recon = get_recon_cost(
        engine_grade, tyre_grade, body_grade, interior_grade, electrical_grade, vendor_type
    )
    wheelr_risk = get_wheelr_risk_deductions(
        owner_count,
        odometer,
        accident_history,
        registration_state,
    ...
    enhanced_max_buy_price = max(
        0,
        int(recommended_buy_price - recon["total"] - wheelr_risk["total"]),
    )
    negotiation = get_negotiation_trio(enhanced_max_buy_price, wheelr_risk["seller_reason_adj"])
    deal_health = get_deal_health(
        market_value,
        recon["total"],
        profit_target,
        owner_count,
...
def evaluate_enhanced(vehicle: EnhancedEvaluateRequest):
    ...
    profit_target = int(
        ...
        or market_value * (target_margin_pct / 100)
    )
