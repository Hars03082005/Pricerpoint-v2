  ...
  return (
    <div className="price-bar-wrap">
      ...
      <div className="price-bar-ends">
        ...
        <span className="price-bar-ci">±{formatINR(mid - min)} range</span>
...
export default function ResultScreen() {
  const { valuationResult, inputs, uploadedImage, conditionScore, setActiveScreen, isLoading } = useApp();
  ...
  const { predictedPrice, baseMarketValue, conditionAdjustment, conditionMultiplier, priceMin, priceMax, damageBoxes, models, warnings, action, confidenceScore, riskScore, riskLevel, dealQualityScore, urgencyScore, recommendedBuyPrice, recommendedSellPrice, expectedProfit, expectedMarginPct, positiveFactors, negativeFactors, isMLPowered, modelName, modelMetrics, valuationSource } = valuationResult;
    ...
    <div className="screen">
      ...
      <div className="result-hero">
        ...
        <div className="result-hero-details">
          <div className="result-hero-name">{inputs.year} {inputs.brand} {inputs.model}</div>
      ...
      <div className="cd-card price-card">
        ...
        <PriceBar min={priceMin} mid={predictedPrice} max={priceMax} />
      ...
      {/* Condition + Damage */}
      <div className="two-col result-inspection-row">
        <div className="cd-card cond-card">
          <div className="cd-section-label">Condition</div>
          <ConditionRing score={displayedConditionScore} />
        </div>
        <div className="cd-card damage-card">
          <div className="cd-section-label">
            <Icon name="eye" size={12} color="#888" strokeWidth={2} />
            Photo Inspection
          </div>
          {uploadedImage ? (
            <div className="damage-img-wrap">
              <img src={uploadedImage.url} alt="Uploaded" className="damage-img" />
              <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="damage-svg">
                {damageBoxes.map((b, i) => (
                  <g key={i}>
                    <rect x={b.x} y={b.y} width={b.w} height={b.h} fill="rgba(247,93,52,0.15)" stroke="#f75d34" strokeWidth="1" strokeDasharray="2 1" rx="0.5" />
                    <text x={b.x+1} y={b.y+5} fill="#f75d34" fontSize="3.5" fontWeight="700">{b.label}</text>
                  </g>
                ))}
              </svg>
              <div className="damage-badge good">CV on hold · manual condition used</div>
            </div>
          ) : (
            <div className="damage-no-photo">
              <Icon name="camera" size={28} color="#ccc" strokeWidth={1.5} />
              <p>No photo uploaded · manual condition used</p>
            </div>
          )}
        </div>
      </div>
